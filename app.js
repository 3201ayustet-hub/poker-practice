...
function renderBeginner(){
  ...
  area.innerHTML = `
    <section class="card beginner-v13">
      <h2>📖 はじめてのポーカー</h2>
      <p class="note">難しい理論より先に、カード・ボード・役の強さをイラストでつかもう。</p>

      <div class="beginner-section hero-beginner">
        <div class="big-shark">🦈✨</div>
        <div>
          <h3>サメ先生とポーカーを始めよう</h3>
          <p>最初から全部覚えなくてOK。まずは「このハンド、意外と強い！」を楽しもう。</p>
        </div>
      </div>

      <div class="beginner-section">
        <h3>① ハンドとボード</h3>
        <p><b>ハンド</b>は自分だけの2枚。<b>ボード</b>は全員が使える場のカードです。</p>
        <div class="rule-illustration">
          <div><span>あなたのハンド</span>${cardHand("A♠K♠")}</div>
          <div><span>フロップ</span>${boardCards("K♦7♣2♥")}</div>
        </div>
        <p class="note">この例ではKが当たっているので「トップヒット」です。</p>
      </div>

      <div class="beginner-section">
        <h3>② 役は5枚で作る</h3>
        <p>手札2枚とボードを組み合わせて、いちばん強い5枚を作ります。</p>
        <div class="rule-illustration">
          <div><span>あなた</span>${cardHand("A♠J♠")}</div>
          <div><span>ボード</span>${boardCards("K♠7♠2♦")}</div>
        </div>
        <p class="note">あと1枚スペードが落ちるとフラッシュ。これがドローの面白さです。</p>
      </div>

      <div class="beginner-section">
        <h3>③ 役の強さと出やすさ</h3>
        <p>強い役ほどなかなか出ません。まずは「ワンペアはよく出る、フルハウス以上はレア」と覚えればOK。</p>
        <div class="hand-rank-table">
          ${renderHandRankRows()}
        </div>
      </div>
      ...
      ${sharkBox("ポーカーは『絶対に勝つ手』がないのが面白いところ。AAでも負けるし、72oでも勝つことがあるんだ！")}
    </section>`;
  renderSurpriseQuiz();
}

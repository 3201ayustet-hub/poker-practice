...
function itemVisual(item){if(!item)return"🎁"; if(item.id.includes("rokuyama"))return"🏆六"; if(item.id.includes("sawada"))return"🏆澤"; if(item.type.includes("サメ"))return"🦈"; if(item.type.includes("カード"))return"🂠"; if(item.type.includes("チップ"))return"●"; if(item.type.includes("テーブル"))return"▰"; if(item.type.includes("ボタン"))return"D"; return"🎁";}
function pullGacha(){const cost=1000;if(state.chips<cost){document.querySelector("#gachaResult").innerHTML=`<div class="gacha-result miss">チップが足りません。クイズ正解でチップを集めよう。</div>`;return;}state.chips-=cost;const items=DATA.gachaItems||[];const total=items.reduce((s,i)=>s+i.weight,0);let roll=Math.random()*total,item=items[0];for(const it of items){roll-=it.weight;if(roll<=0){item=it;break;}}const isNew=!state.collection.includes(item.id);if(isNew)state.collection.push(item.id);saveLabProgress();document.querySelector("#gachaResult").innerHTML=`<div class="gacha-result rarity-${item.rarity}"><div class="gacha-big">${itemVisual(item)}</div><h3>${rarityIcon(item.rarity)} ${item.rarity} ${item.name}</h3><p>${item.type}${isNew?" / NEW!":" / 入手済み"}</p><small>${item.rarity==="LEGEND"?"伝説のチップだ！！":"コレクションに追加されました。"}</small></div>`;const chips=document.querySelector("#labChips");if(chips)chips.textContent=state.chips.toLocaleString();const cc=document.querySelector("#collectionCount");if(cc)cc.textContent=collectionCount();}
function todaySharkMaxim(){const list=DATA.sharkMaxims||DATA.sharkQuotes||["今日もレンジを鍛えるぞ！"];return list[Math.floor(Math.random()*list.length)];}
...
function renderHome(){
  ...
  document.querySelector("#modeTitle").textContent="SHARK POKER LAB";document.querySelector("#progress").textContent="-";document.querySelector("#score").textContent="-";
...
function renderGacha(){
  ...
  document.querySelector("#refArea").innerHTML=`<section class="card gacha-room"><div class="gacha-head"><div><p class="lab-kicker">COLLECTION VAULT</p><h2>🎰 Labガチャ</h2><p class="note">クイズで集めたチップで、見た目アイテムをコレクションできます。</p></div><div class="lab-wallet compact"><span>所持チップ</span><b><i class="chip-icon"></i><em id="labChips">${Number(state.chips||0).toLocaleString()}</em></b><small>Collection <em id="collectionCount">${collectionCount()}</em></small></div></div><div class="gacha-machine"><div class="machine-window">🦈🎰♠</div><button class="pull-btn" onclick="pullGacha()">1000チップで引く</button><div id="gachaResult"></div></div><div class="rarity-strip">${rarities.map(r=>`<span class="rarity-${r}">${rarityIcon(r)} ${r}</span>`).join("")}</div><div class="collection-grid">${items.map(item=>`<div class="collection-card ${owned.has(item.id)?"owned":"locked"} rarity-${item.rarity}"><div class="collection-visual">${owned.has(item.id)?itemVisual(item):"?"}</div><b>${owned.has(item.id)?item.name:"未獲得"}</b><small>${item.rarity} / ${item.type}</small></div>`).join("")}</div></section>`;
}

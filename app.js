...
function renderQuiz(){
  ...
  document.querySelector("#nextBtn").classList.add("next-disabled");
...
function answer(num, clicked){
  ...
  document.querySelector("#nextBtn").classList.remove("next-disabled");
  document.querySelector("#nextBtn").onclick = nextQuestion;
  document.querySelector("#nextBtn").onclick = nextQuestion;
...
function renderBeginner(){
  ...
  area.innerHTML = `
    <section class="card beginner-v13">
      <h2>📖 はじめてのポーカー</h2>
      <p class="note">まずは「7枚の中から、いちばん強い5枚を作る」と覚えよう。</p>
      <div class="beginner-section hero-beginner">
        <div class="big-shark">🦈✨</div>
        <div><h3>サメ先生とポーカーを始めよう</h3><p>難しい言葉はあとでOK。まずはカードを見て、どの5枚が役になるかを楽しもう。</p></div>
      </div>
      <div class="beginner-section">
        <h3>① ハンドとボード</h3>
        <p><b>ハンド</b>は自分だけの2枚。<b>ボード</b>は全員が使える5枚です。</p>
        <div class="seven-card-demo">
          <div><span>あなたのハンド</span><div class="demo-cards">${demoCard("A","♠",true)}${demoCard("K","♠",true)}</div></div>
          <div><span>ボード5枚</span><div class="demo-cards">${demoCard("Q","♠",true)}${demoCard("J","♠",true)}${demoCard("T","♠",true)}${demoCard("7","♣",false)}${demoCard("2","♥",false)}</div></div>
        </div>
        <p class="note">光っている5枚が完成役。A♠K♠Q♠J♠T♠で最強の役になります。</p>
      </div>
      <div class="beginner-section">
        <h3>② 7枚から強い5枚を選ぶ</h3>
        <p>手札2枚を必ず両方使う必要はありません。7枚の中から、いちばん強い5枚を選びます。</p>
        <div class="seven-card-demo">
          <div><span>あなたのハンド</span><div class="demo-cards">${demoCard("8","♣",true)}${demoCard("8","♦",true)}</div></div>
          <div><span>ボード5枚</span><div class="demo-cards">${demoCard("8","♠",true)}${demoCard("K","♥",true)}${demoCard("K","♣",true)}${demoCard("4","♦",false)}${demoCard("2","♣",false)}</div></div>
        </div>
        <p class="note">光っている5枚で、8が3枚＋Kが2枚。かなり強い役です。</p>
      </div>
      <div class="beginner-section">
        <h3>③ 役の強さと出やすさ</h3>
        <p>強い役ほどなかなか出ません。まずは「ワンペアはよく出る、フルハウス以上はレア」と覚えればOK。</p>
        <div class="hand-rank-table">${renderHandRankRows()}</div>
      </div>
      <div class="beginner-section">
        <h3>④ まず覚えるポジション</h3>
        <p><b>BTNは有利</b>、<b>UTGは慎重</b>。最初はこれだけで十分です。</p>
        <div class="poker-table mini"><div class="pos utg">UTG</div><div class="pos hj">HJ</div><div class="pos co">CO</div><div class="pos btn">BTN</div><div class="pos sb">SB</div><div class="pos bb">BB</div><div class="dealer">♠ ♥ ♦ ♣</div></div>
      </div>
      <div class="beginner-section">
        <h3>⑤ サメ先生の驚きクイズ</h3>
      ...
      <div class="beginner-section">
        <h3>⑥ このアプリのおすすめ順</h3>
        <ol class="start-steps"><li>まず驚きクイズで遊ぶ</li><li>プリフロップクイズでレンジを覚える</li><li>勝率感覚クイズでハンドの強さをつかむ</li><li>ボード事典でフロップの考え方を見る</li></ol>
      </div>
...
function demoCard(rank, suit, active){
  return `<span class="mini-card ${suitClass(suit)} ${active ? "best-five" : "unused-card"}"><span class="rank">${rank}</span><span class="suit">${suit}</span></span>`;
}
function renderHandRankRows(){
  const ranks = [
    ["ロイヤルフラッシュ","約0.00015%","A-K-Q-J-Tの同じスート。幻級。"],
    ["ストレートフラッシュ","約0.0014%","同じスートで数字が連続。"],
    ["フォーカード","約0.024%","同じ数字が4枚。かなりレア。"],
    ["フルハウス","約0.14%","スリーカード＋ワンペア。"],
    ["フラッシュ","約0.20%","同じスートが5枚。"],
    ["ストレート","約0.39%","数字が5枚連続。"],
    ["スリーカード","約2.1%","同じ数字が3枚。"],
    ["ツーペア","約4.8%","ペアが2組。"],
    ["ワンペア","約42%","同じ数字が2枚。よく出る。"],
    ["ハイカード","約50%","役なし。高いカード勝負。"]
  ];
  return ranks.map((r,i)=>`<div class="rank-row"><b>${i+1}. ${r[0]}</b><span>${r[1]}</span><p>${r[2]}</p></div>`).join("");
}
